...
export async function getEachVideoDurationArray(playlistAllVideosIdArray , API_KEY) {
...
export function getTotalTimeDuration(eachVideoDurationArray,  fromVidNum, toVidNum , totalVideosInPlaylist,) {
    ...
    if (fromVidNum !== 0 && fromVidNum <= totalVideosInPlaylist && fromVidNum > 0) {
    ...
    for (let i = lowerIndxFrom; i <= upperIndexTo; i++) {
        ...
        if (mindx !== -1) {
            min += parseInt(time.slice(hrsidx !== -1 ? hrsidx + 1 : tidx + 1, mindx), 10);
        }

        if (secidx !== -1) {
            sec += parseInt(time.slice(mindx !== -1 ? mindx + 1 : (hrsidx !== -1 ? hrsidx + 1 : tidx + 1), secidx), 10);
        }
    }
